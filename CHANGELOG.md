...
- Locked the dual-fingerprint (fingerprint-secret rotation) architecture — architecture gate only, no runtime implementation. Added `previousFingerprintHash` (nullable, additive, end of `DeviceIdentityDTO`) to the identity contract while `DeviceIdentityResolverInterface::resolve()` stays unchanged; the default resolver hashes the same normalized raw identity once with the current hasher and once with an optional previous hasher; device-derived V1 keys pair the previous outer secret with `previousFingerprintHash` (write-current/read-previous, no cross-pairing, outer-key-only rotation remains compatible); K5 micro-cap becomes V1/V2 with V2-authoritative atomic seeding and no `max(v1,v2)` merge. Dual-fingerprint public contract = architecture locked, runtime pending. K3/K5 true fingerprint-secret rotation and K5 micro-cap true fingerprint-secret rotation = pending. Correlation/ephemeral fingerprint-secret rotation = pending separate design (not claimed). Normative ownership: `docs/DEVICE_FINGERPRINT.md` §5.1, `docs/KEY_STRATEGY.md` §4.3.3 / §4.5.2.
